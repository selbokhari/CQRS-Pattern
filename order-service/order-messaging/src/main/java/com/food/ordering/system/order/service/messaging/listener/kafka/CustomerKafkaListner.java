...
@Component
@AllArgsConstructor
public class CustomerKafkaListner implements KafkaConsumer<CustomerAvroModel> {
    ...
    public void receive(@Payload List<CustomerAvroModel> messages,
        ...
        log.info("{} number of customers received with keys:{}, partitions:{} and offsets: {}",
                messages.size(),
                keys.toString(),
                partitions.toString(),
                offsets.toString());
        messages.stream()
                .map(orderMessagingDataMapper::customerAvroModelToCustomerModel)
                .forEach(customerMessageListener::customerCreated);
    }
}
